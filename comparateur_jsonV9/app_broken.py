...
from exceptions import (
    ...
    TranslationError, UIError, ConfigurationError
)
from error_utils import (
    safe_execute, safe_ui_operation, safe_file_operation,
    validate_json_structure, robust_widget_destroy
)
...
class FaultEditor:
    ...
    def reload_root(self, event=None):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Impossible de restaurer le chemin précédent : {e}")
                ...
                logger.warning(f"Erreur UI lors de la restauration : {e}")
                print(f"❌ Erreur UI lors de la restauration du chemin : {e}")
                self.status.config(text="✅ Interface rechargée (racine)")
        except (FileOperationError, JSONValidationError, UIError) as e:
            logger.error(f"Erreur métier lors du rechargement : {e}")
            print(f"❌ Erreur lors du rechargement : {e}")
            self.status.config(text="❌ Erreur de rechargement")
        except tk.TclError as e:
            logger.error(f"Erreur Tkinter lors du rechargement : {e}")
            print(f"❌ Erreur UI lors du rechargement : {e}")
            self.status.config(text="❌ Erreur de rechargement")
        except (OSError, MemoryError, RecursionError) as e:
            logger.error(f"Erreur système lors du rechargement : {e}")
            print(f"❌ Erreur système lors du rechargement : {e}")
            self.status.config(text="❌ Erreur de rechargement")
        except Exception as e:
            logger.error(f"Erreur inattendue lors du rechargement : {e}")
            print(f"❌ Erreur lors du rechargement : {e}")
            self.status.config(text="❌ Erreur de rechargement")
            raise UIError(f"Rechargement impossible: {e}") from e
    ...
    def run_coherence_check_step(self, dossier_base, apply_fix):
        ...
        try:
            ...
            print(f"❌ Erreur subprocess lors de la vérification de cohérence : {e}")
            return {'success': False, 'output': '', 'errors': str(e)}
        except FileNotFoundError as e:
            logger.error(f"Script check_coherence.py introuvable : {e}")
            print(f"❌ Script check_coherence.py introuvable : {e}")
            return {'success': False, 'output': '', 'errors': str(e)}
        except PermissionError as e:
            logger.error(f"Erreur d'accès lors de la vérification : {e}")
            print(f"❌ Erreur d'accès lors de la vérification de cohérence : {e}")
            return {'success': False, 'output': '', 'errors': str(e)}
        except (OSError, UnicodeDecodeError, TimeoutError) as e:
            logger.error(f"Erreur système lors de la vérification de cohérence : {e}")
            print(f"❌ Erreur système lors de la vérification de cohérence : {e}")
            return {'success': False, 'output': '', 'errors': str(e)}
        except Exception as e:
    ...
    def run_spelling_check_step(self, dossier_base):
        ...
        try:
            ...
            logger.error(f"Erreur inattendue lors de la vérification orthographique : {e}")
    ...
    def run_headers_fix_step(self, dossier_base):
        ...
        try:
            ...
            logger.error(f"Erreur système lors de la correction des headers : {e}")
            print(f"❌ Erreur système lors de la correction des headers : {e}")
            return {'success': False, 'output': '', 'errors': str(e), 'fixed': False}
        except Exception as e:
    ...
    def reload_data(self):
        ...
        for filename, filepath in self.path_map.items():
            try:
                ...
                logger.error(f"Fichier JSON introuvable lors du rechargement : {filepath} - {e}")
                ...
                logger.error(f"Erreur d'accès lors du rechargement de {filename}: {e}")
    ...
    def sync_files(self):
        ...
        try:
            ...
            self.status.config(text="❌ Erreur subprocess lors de la synchronisation")
            print(f"\n❌ Erreur subprocess lors de la synchronisation : {e}")
        except FileNotFoundError as e:
            self.status.config(text="❌ Script ou fichier introuvable")
            print(f"\n❌ Script ou fichier introuvable : {e}")
        except PermissionError as e:
            self.status.config(text="❌ Erreur d'accès lors de la synchronisation")
            print(f"\n❌ Erreur d'accès lors de la synchronisation : {e}")
        except (OSError, UnicodeDecodeError) as e:
            self.status.config(text="❌ Erreur système lors de la synchronisation")
    ...
    def load_level(self, path, level):
        ...
        try:
            ...
            logger.error(f"Erreur JSON dans {filename}: {str(e)}")
            self.status.config(text=f"❌ JSON invalide dans {filename}")
            return
        except PermissionError:
            logger.error(f"Accès refusé au fichier : {filename}")
            self.status.config(text=f"❌ Accès refusé : {filename}")
            return
        except UnicodeDecodeError as e:
            logger.error(f"Erreur d'encodage dans {filename}: {str(e)}")
            self.status.config(text=f"❌ Erreur d'encodage : {filename}")
            return
        except (OSError, MemoryError) as e:
            logger.error(f"Erreur système lors de la lecture de {filename}: {str(e)}")
            self.status.config(text=f"❌ Erreur système : {filename}")
            return
        except Exception as e:
            logger.error(f"Erreur inattendue lors de la lecture de {filename}: {str(e)}")
    ...
    def save_file(self, rel_path):
        ...
        try:
            ...
            logger.error(f"Fichier de destination introuvable : {rel_path}")
            self.status.config(text=f"❌ Fichier introuvable : {rel_path}")
        except PermissionError:
            logger.error(f"Accès refusé pour la sauvegarde : {rel_path}")
            self.status.config(text=f"❌ Accès refusé : {rel_path}")
        except KeyError as e:
            logger.error(f"Données manquantes pour {rel_path}: {str(e)}")
            self.status.config(text=f"❌ Données manquantes : {rel_path}")
        except OSError as e:
            logger.error(f"Erreur système lors de la sauvegarde {rel_path}: {str(e)}")
            self.status.config(text=f"❌ Erreur système : {rel_path}")
        except (UnicodeEncodeError, UnicodeDecodeError) as e:
            logger.error(f"Erreur d'encodage lors de la sauvegarde {rel_path}: {str(e)}")
            self.status.config(text=f"❌ Erreur d'encodage : {rel_path}")
        except MemoryError as e:
            logger.error(f"Mémoire insuffisante pour sauvegarder {rel_path}: {str(e)}")
            self.status.config(text=f"❌ Mémoire insuffisante : {rel_path}")
        except Exception as e:
    ...
    def show_flat_json_editor(self, all_keys, translations, fr_path, en_path, es_path):
        ...
        for row_idx, key in enumerate(all_keys, start=1):
            ...
            key_label.grid(row=row_idx, column=0, sticky="ew", padx=2, pady=3)            # Colonnes traductions
    ...
    def translate_row(self, editor_window, row):
        ...
        if fr_text and fr_text.get().strip():
            try:
                ...
                logger.error(f"Erreur de traduction pour la ligne {row}: {e}")
                print(f"Erreur de traduction de la ligne {row}: {e}")
                ...
                logger.error(f"Erreur système lors de la traduction de la ligne {row}: {e}")
                print(f"Erreur système lors de la traduction de la ligne {row}: {e}")
                # Effet visuel d'erreur
                for widget in editor_window.grid_frame.grid_slaves(row=row):
                    widget.config(bg=COL_RED)
                    editor_window.after(500, lambda w=widget: w.config(
                        bg=COL_BG_ROW if row % 2 == 1 else COL_BG_ROW_ALT))
                if hasattr(editor_window, 'status_bar'):
                    editor_window.status_bar.config(text=f"❌ Erreur système ligne {row}")
            except (ConnectionError, TimeoutError) as e:
                logger.error(f"Erreur de connexion lors de la traduction de la ligne {row}: {e}")
                print(f"Erreur de connexion lors de la traduction de la ligne {row}: {e}")
                # Effet visuel d'erreur
                for widget in editor_window.grid_frame.grid_slaves(row=row):
                    widget.config(bg=COL_RED)
                    editor_window.after(500, lambda w=widget: w.config(
                        bg=COL_BG_ROW if row % 2 == 1 else COL_BG_ROW_ALT))
                if hasattr(editor_window, 'status_bar'):
                    editor_window.status_bar.config(text=f"❌ Erreur de connexion ligne {row}")
    ...
    def next_flat_search_result(self, editor_window):
        ...
        if not editor_window.search_results:
            return        editor_window.current_search_index = (editor_window.current_search_index + 1) % len(editor_window.search_results)
    ...
    def translate_text(self, text, target_lang):
        ...
        try:
            ...
            logger.error(f"Erreur de traduction spécifique: {e}")
            ...
            logger.error(f"Erreur d'encodage lors de la traduction: {e}")
            print(f"Erreur d'encodage lors de la traduction: {e}")
            return text
        except OSError as e:
            logger.error(f"Erreur système lors de la traduction: {e}")
            print(f"Erreur système lors de la traduction: {e}")
            return text
