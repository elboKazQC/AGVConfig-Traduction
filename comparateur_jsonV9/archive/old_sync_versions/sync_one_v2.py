...
def special_translations(text, target_lang):
    """Gère les cas spéciaux de traduction qui nécessitent des règles particulières."""
      # Liste de traductions spéciales pour le mot "balayeur"
    translations = {
    ...
    # Vérifie si le texte (en minuscules) contient le mot "balayeur"
    text_lower = text.lower()
    for fr_term, translations in SPECIAL_CASES.items():
        if fr_term in text_lower:
            # Si on trouve une correspondance, on retourne la traduction spéciale
